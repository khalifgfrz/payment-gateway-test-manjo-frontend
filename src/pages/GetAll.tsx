...
export default function GetAll() {
  ...
  const filteredTransactions = transactions.filter((trx) => {
    ...
    return trx.reference_no?.toLowerCase().includes(searchLower) || trx.merchant_id?.toLowerCase().includes(searchLower) || trx.trx_id?.toLowerCase().includes(searchLower) || trx.status?.toLowerCase().includes(searchLower);
  });
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="card p-6 mb-6">
        <div className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1">
            ...
            <input type="text" placeholder="Cari berdasarkan Reference No, Merchant ID, Trx ID, atau Status..." value={filter} onChange={(e) => setFilter(e.target.value)} className="input" />
          </div>

          <button onClick={() => fetchAllTransactions(1)} disabled={loading} className="btn px-6">
      ...
      {error && (
        <div className="card p-4 bg-red-50 border border-red-200">
          ...
          <button onClick={() => fetchAllTransactions(1)} className="mt-3 px-4 py-2 bg-red-600 text-white rounded-lg text-sm hover:bg-red-700 transition-colors">
            Coba Lagi
          </button>
        </div>
      )}
      ...
      {!loading && !error && filteredTransactions.length === 0 && (
        <div className="card p-12 text-center border-2 border-dashed border-slate-300">
          ...
          <p className="text-slate-600 mb-2">{filter ? "Tidak ada transaksi yang cocok dengan pencarian" : "Tidak ada transaksi"}</p>
          {filter && (
            <button onClick={() => setFilter("")} className="text-blue-600 hover:text-blue-700 text-sm font-medium">
              Bersihkan Filter
            </button>
          )}
        </div>
          ...
          <div className="bg-slate-50 px-6 py-4 border-t border-slate-200">
            <div className="flex items-center justify-between">
              <p className="text-sm text-slate-600">
                Halaman <span className="font-semibold">{currentPage}</span> dari <span className="font-semibold">{meta.total_pages}</span> • Total <span className="font-semibold">{meta.total}</span> transaksi
              </p>
                ...
                {Array.from({ length: Math.min(meta.total_pages, 5) }, (_, i) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={loading}
                      className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${page === currentPage ? "bg-blue-600 text-white" : "bg-slate-200 text-slate-900 hover:bg-slate-300"} disabled:opacity-50`}
                    >
                      {page}
                    </button>
                  );
